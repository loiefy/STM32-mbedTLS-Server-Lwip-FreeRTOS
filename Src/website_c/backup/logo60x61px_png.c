#include "lvgl/lvgl.h"

#ifndef LV_ATTRIBUTE_MEM_ALIGN
#define LV_ATTRIBUTE_MEM_ALIGN
#endif

#ifndef LV_ATTRIBUTE_IMG_LOGO60X61PX_PNG
#define LV_ATTRIBUTE_IMG_LOGO60X61PX_PNG
#endif

const LV_ATTRIBUTE_MEM_ALIGN LV_ATTRIBUTE_IMG_LOGO60X61PX_PNG uint8_t logo60x61px_png_map[] = {
  , 
};

const lv_img_dsc_t logo60x61px_png = {
  .header.always_zero = 0,
  .header.w = 60,
  .header.h = 61,
  .data_size = 3804,
  .header.cf = LV_IMG_CF_RAW,
  .data = logo60x61px_png_map,
};
